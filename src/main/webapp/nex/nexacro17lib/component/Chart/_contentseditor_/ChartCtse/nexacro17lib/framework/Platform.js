...
if (!nexacro._bInitPlatform) {
	...
	_pEnvironment.set_datasetrule = function (v) {
		if (nexacro._is_loaded_application) {
			return;
		}

		if (v) {
			nexacro._datasetrule = this.datasetrule = nexacro._toString(v);
			this.on_apply_datasetrule();
		}
	};

	_pEnvironment.on_apply_datasetrule = function () {
		var v = this.datasetrule;
		switch (v) {
			case "2.0":
